...
class _WardenNoticeScreenState extends State<WardenNoticeScreen> {
  ...
  bool _isVisibleToWarden(Map<String, dynamic> data) {
    final isActive = data['isActive'] as bool? ?? true;
    if (!isActive) return false;
    final roles = data['audienceRoles'];
    if (roles is! List) return true;
    final audience = roles.whereType<String>().toList();
    return audience.contains('warden') || audience.contains('admin');
  }
      ...
      child: Scaffold(
        ...
        appBar: PsgGlassAppBar(
          leading: IconButton(
            ...
            onPressed: () =>
                context.canPop() ? context.pop() : context.go('/warden'),
          ),
          title: 'Notices',
        ),
        ...
        body: StreamBuilder<QuerySnapshot>(
          ...
          builder: (context, snapshot) {
            ...
            final docs = (snapshot.data?.docs ?? [])
                .where((doc) =>
                    _isVisibleToWarden(doc.data() as Map<String, dynamic>))
                .toList();
            if (docs.isEmpty) {
              return Center(
                child: Column(
