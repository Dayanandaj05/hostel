...
class _TShirtListScreenState extends State<TShirtListScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (uid != null) context.read<TShirtController>().startWatchingOrders(uid);
    });
  }
      ...
      body: Consumer<TShirtController>(
        builder: (context, controller, _) {
            ...
            return Center(
              child: Column(
                ...
                children: [
                  ...
                  Text('No orders yet', style: TextStyle(color: Colors.grey.shade500, fontSize: 18)),
                ],
              ),
            );
          }
          return ListView.builder(
            ...
            itemBuilder: (_, i) => _OrderCard(order: controller.myOrders[i]),
          );
        },
      ),
    );
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          Container(width: 1, height: 120, color: Colors.grey.shade200, margin: const EdgeInsets.symmetric(vertical: 10)),
          Expanded(
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  Text(order.type, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16, color: Color(0xFF0D2137))),
                  const SizedBox(height: 6),
                  Row(
                    children: [
                      _pill('Size: ${order.size}', const Color(0xFF009688)),
                      const SizedBox(width: 8),
                      _pill('Qty: ${order.quantity}', Colors.indigo),
                    ],
                  ),
                  const SizedBox(height: 8),
                  Text('₹${order.totalPrice.toStringAsFixed(0)}',
                    style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 15, color: Color(0xFF0D2137))),
                  if (order.createdAt != null) ...[
                    const SizedBox(height: 6),
                    Text(_fmt(order.createdAt!), style: TextStyle(color: Colors.grey.shade500, fontSize: 11)),
                  ],
                ],
              ),
            ),
          ...
          Padding(
            ...
            child: Icon(
              order.status == 'delivered' ? Icons.check_circle_rounded : Icons.qr_code_2_rounded,
