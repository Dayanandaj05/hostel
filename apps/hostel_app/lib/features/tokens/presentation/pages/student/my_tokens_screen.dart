...
import '../../controllers/food_token_controller.dart';
...
class _MyTokensScreenState extends State<MyTokensScreen>
      ...
      body: Consumer<FoodTokenController>(
        builder: (context, controller, _) {
          ...
          return TabBarView(
            controller: _tabController,
            children: [
              _buildTokenList(active, emptyMessage: 'No active tokens'),
              _buildTokenList(history, emptyMessage: 'No token history'),
            ],
          );
        },
      ),
  ...
  Widget _buildTokenList(List<FoodTokenModel> tokens, {required String emptyMessage}) {
    if (tokens.isEmpty) {
      return Center(
        child: Column(
...
class _TokenCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    String mealSlot = token.mealSlot ?? '';
    final mealIcon = switch (mealSlot.toLowerCase()) {
      ...
      child: Row(
        children: [
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Row(
                    children: [
                      _pill(Icons.confirmation_number_outlined,
                          'Qty: ${token.quantity ?? 1}', Colors.indigo),
          ...
          Padding(
            ...
            child: Icon(
              isActive ? Icons.qr_code_2_rounded : Icons.check_circle_rounded,
              color: statusColor,
              size: 32,
            ),
          ),
